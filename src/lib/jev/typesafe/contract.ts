...
export type ContractError = {
  kind:
    ...
    | "score-type"
    | "unknown-type"
    /** An answer id that is not one of the emitted question ids. */
    | "unknown-question"
    /** `opts.loadBearing` narrowed or emptied the default set. */
    | "load-bearing-set"
    /** The state blob does not carry what the questions reference. */
    | "state-shape";
  id?: string;
  message: string;
};
